...
namespace AdventOfCode.Solutions.Year2020
{
    class CupGame
    {
        public LinkedList<int> cups { get; set; } = new();
        private LinkedListNode<int>? currentCup { get; set; }
        public int highest { get; set; }

        // Keep a record of all nodes
        private Dictionary<int, LinkedListNode<int>> references { get; set; } = new();
        ...
        private void addCup(int value)
        {
            ...
            if (this.cups.Last != default)
                this.references.Add(value, this.cups.Last);
        }

        public void playRound()
        {
            if (this.currentCup == default)
                return;
            ...
            LinkedListNode<int>? destinationCup = default;

            while (destinationCup == default)
            {
            ...
            foreach (var i in hand)
            {
                ...
                if (destinationCup.Next != default)
